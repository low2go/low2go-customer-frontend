    ...
    public async placeOrder(
      ...
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(
            `Failed to place order: ${response.status} ${response.statusText} - ${errorDetails}`
          );
        }
    
        // Log explicitly if the status is 201 (Created) with no body
        if (response.status === 201) {
          console.log('Order successfully placed. No response body was provided.');
          return response; // Optionally return the response object if required
        }
